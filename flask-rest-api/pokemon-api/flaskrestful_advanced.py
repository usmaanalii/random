...
def list_of_strings(value):
    all_str = isinstance(value, list) and all(
...
def get_pokemon_args():
    ...
    parser.add_argument('type', type=list_of_strings, location='json')
    parser.add_argument('weakenesses', type=list_of_strings, location='json')
    parser.add_argument('evolutions', type=list, location='json')
    ...
    for evolution in root_args.get('evolutions'):
        ...
        evolutions_parser.add_argument(
            'name', required=True, type=str, location='evolution')
        evolutions_parser.add_argument(
            'number', required=True, type=int, location='evolution')

        evolution_arg = reqparse.Namespace()
        evolution_arg.evolution = evolution
        evolutions_parser.parse_args(req=evolution_arg, strice=True)
...
class Pokemon(Resource):
    def get(self):
        ...
        parser.add_argument('number', type=false,
    ...
    def delete(self):
        ...
        pokedex[:] = [
            pokemon for pokemon in pokedex if pokemon.get('number') != number]
        success = True
